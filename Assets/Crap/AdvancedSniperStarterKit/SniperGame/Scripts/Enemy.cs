...
public class Enemy : MonoBehaviour
{
	...
	void Start ()
	{
		...
		Myself.animation.CrossFade (IdlePose, 0.3f);
	}
	...
	void Update ()
	{
		...
		switch (state) {
		case 0:
			Myself.animation.CrossFade (RunPose, 0.3f);
	...
	public bool GroundChecking ()
	{
		if (collider) {
			...
			if (characterController.isGrounded)
				return true;
			if (Physics.Raycast (collider.bounds.center, -Vector3.up, out hit, DistanceToGround + 0.1f)) {
				return true;
			}
		}
		return false;
